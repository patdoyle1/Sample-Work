...
class DistancePixel {
 public:
  DistancePixel() : x(-1),y(-1),distance(std::numeric_limits<double>::max()), hasFinalValue(false) {}
  // accessor
  int getX() const { return x; }
  int getY() const { return y; }
  double getValue() const { return distance; }
  const bool isFinalValue() const { return hasFinalValue; }
  // modifier
  void setX(int _x) { x = _x; }
  void setY(int _y) { y = _y; }
  void setValue(double v) { distance = v; }
  void makeFinalValue() { hasFinalValue = true; }
 private:
  // REPRESENTATION
...
public:
  ...
  T& GetPixel(int x, int y) {
    //std::cout << "x-val is: "<<x << ", width is: " << width << std::endl;
    assert(x >= 0 && x < width);
    assert(y >= 0 && y < height);
    return data[y*width + x]; }
